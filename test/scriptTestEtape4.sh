...
echo ********************************************
echo * Lancement des tests de la partie 4	*
echo ********************************************
echo 
...
do
        ...
        then
                ...
                ./nachos-step4 -rs 2 -x  $files > /tmp/resultat_test_Partie4_etape2
...
do
        ...
        then
                ...
                ./nachos-step4 -rs 2 -x  $files > /tmp/resultat_test_Partie4_etape3
...
###########################################################################################################################################################################
#on change de dossier pour faire les test de l'etape4

`rm $directory/*.o`
PWD2=`pwd`
directory=$PWD2/etape4/
#on efface les .o pour eviter des problèmes.
`rm $directory*.o`
#on lance notre recherche pour avoir les noms des fichiers executable
command=`find $directory -executable` 
...
do
        ...
        then
                ...
                ./nachos-step4 -rs 2 -x  $files > /tmp/resultat_test_Partie4_etape4
                cat /tmp/resultat_test_Partie3_etape3
                echo ''
		read -p "Passer au test suivant ? [O/N]" rep
		case $rep in
